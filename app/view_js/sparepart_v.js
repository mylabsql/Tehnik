valid_script = true;  
part_ajax_url = 'ajax.handler.php?id=' + page;  

function reportSelect(bt){
  dynamic_grid_reg_part01.getTopToolbar().get(8).setText(bt.text); 
  dynamic_grid_reg_part01.getTopToolbar().get(8).mode = bt.mode;   
}
var dspart01 = new Ext.data.JsonStore({
	id:dspart01,
	url:part_ajax_url,
	totalProperty:'total',
	baseParams:{
		action:'getcmbnama01',
		thisfilter:'Sparepart'
	},
	sortInfo:{
		field:'nama',
		direction:'ASC'
...
var cmbpart01 = new Ext.form.ComboBox(
{	
	anchor:'100%',
	store:dspart01,
	//name:'id',
	...
	hiddenValue : 'id',
	hiddenName : 'nama_id',
	fieldLabel: 'Nama sparepart',
	emptyText : 'Nama sparepart...',
	triggerAction: 'all',
	mode: 'remote',
	editable: true,
	autocomplete: true,
...
var dynamic_grid_reg_part01 = new Ext.ux.DynamicGroupingGrid({
    border:false,
    remoteSort:true, //optional default true
    autoLoadStore:true, //optional default true
    storeUrl:part_ajax_url,
    ...
    '-',{
      ...
      mode:'pdf',
      handler:function(){
        options = dynamic_grid_reg_part01.getParamsFilter();
        report_link = 'report.php?id=' + page;
        options = Ext.apply(options,{mode:this.mode}); 
        winReport({
            id : this.id,
            title : 'Informasi Nama Sparepart',
            url : report_link,
            type : this.mode,
            params:options        
        }); 
    ...
    onAddData:function(bt){
      Ext.getCmp('win_part01').getForm().reset();
      win_reg_part01.setTitle('Registrasi sparepart'); 
      win_reg_part01.show(bt.id); 
    },
    onEditData:function(bt,rec){
      win_reg_part01.setTitle('Ubah informasi sparepart');
      win_reg_part01.show(bt.id); 
      Ext.getCmp('win_part01').getForm().load({
...
win_reg_part01 = new Ext.Window({
  ...
  plain:true,
  frame:true,
  items:[userPic,win_part01],
  buttons:[
  {
    text:'Save',
    handler:function()
    {	if(!Ext.getCmp('win_part01').getForm().isValid()){
        Ext.example.msg('Peringatan','Ada data yang kosong'); 
      }
      
      id_data = Ext.getCmp('win_part01').getForm().getValues().id; 
      action = (id_data?'update':'create'); 
      Ext.getCmp('win_part01').getForm().submit({
          params:{action:action,thisfilter:'Sparepart',
	  usr_id:userid},
          waitMsg : 'Saving Data',
          success:function(){
            win_reg_part01.hide();
            Ext.example.msg('Simpan','Data telah disimpan'); 
            dynamic_grid_reg_part01.store.reload(); 
          },
...
var main_content = {
  ...
  title:n.text,  
  iconCls:n.attributes.iconCls,  
  items : [dynamic_grid_reg_part01],
  listeners:{
    destroy:function(){
      my_win = Ext.getCmp('win-reg-part01');
      if (my_win)
          my_win.destroy(); 
    }
  }
