valid_script = true;  
grup_ajax_url = 'ajax.handler.php?id=' + page;  

function reportSelect(bt){
  dynamic_grid_reg_grup01.getTopToolbar().get(8).setText(bt.text); 
  dynamic_grid_reg_grup01.getTopToolbar().get(8).mode = bt.mode;   
}
var dsgrup01 = new Ext.data.JsonStore({
	id:dsgrup01,
	url:grup_ajax_url,
	totalProperty:'total',
	baseParams:{
...
var dynamic_grid_reg_grup01 = new Ext.ux.DynamicGroupingGrid({
    border:false,
    remoteSort:true, //optional default true
    autoLoadStore:true, //optional default true
    storeUrl:grup_ajax_url,
    ...
    '-',{
      ...
      mode:'pdf',
      handler:function(){
        options = dynamic_grid_reg_grup01.getParamsFilter();
        report_link = 'report.php?id=' + page;
        options = Ext.apply(options,{mode:this.mode}); 
        winReport({
            id : this.id,
            title : 'Informasi Nama grup',
            url : report_link,
            type : this.mode,
            params:options        
        }); 
    ...
    onAddData:function(bt){
      Ext.getCmp('win_grup01').getForm().reset();
      win_reg_grup01.setTitle('Registrasi grup'); 
      win_reg_grup01.show(bt.id); 
    },
    onEditData:function(bt,rec){
      win_reg_grup01.setTitle('Ubah informasi grup');
      win_reg_grup01.show(bt.id); 
      Ext.getCmp('win_grup01').getForm().load({
...
win_reg_grup01 = new Ext.Window({
  ...
  plain:true,
  frame:true,
  items:[userPic,win_grup01],
  buttons:[
  {
    text:'Save',
    handler:function()
    {	if(!Ext.getCmp('win_grup01').getForm().isValid()){
        Ext.example.msg('Peringatan','Ada data yang kosong'); 
      }
      
      id_data = Ext.getCmp('win_grup01').getForm().getValues().id; 
      action = (id_data?'update':'create'); 
      Ext.getCmp('win_grup01').getForm().submit({
          params:{action:action,thisfilter:'grup',
	  usr_id:userid},
          waitMsg : 'Saving Data',
          success:function(){
            win_reg_grup01.hide();
            Ext.example.msg('Simpan','Data telah disimpan'); 
            dynamic_grid_reg_grup01.store.reload(); 
          },
...
var main_content = {
  ...
  title:n.text,  
  iconCls:n.attributes.iconCls,  
  items : [dynamic_grid_reg_grup01],
  listeners:{
    destroy:function(){
      my_win = Ext.getCmp('win-reg-grup01');
      if (my_win)
          my_win.destroy(); 
    }
  }
