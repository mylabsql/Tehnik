<?php
class tgequ01 extends msDB { //class Hwd dengan extend/parameter msDB//
	private $text = "";
	/**
	...
	/**
	 * Getting Text Equ Recursive
	 *
	 * @param integer $parent_id
	 * @param string $indent
	 * @return recordset
	 */
	function getEquText($parent_id, $indent) {
		$rs = $this->getChild($parent_id);
		while ($row=$rs->FetchNextObject()) {
			$this->text.= $indent.$row->TITLE."<br>";
			$this->getEquText($row->MENU_ID, $indent."=>");			
		}
		return $this->text; 
	}
	 ...
	 * @return string
	 */
	function getAllEqu($root_title='SCTV',$mode='json', $publish=0,$check=0,$group=-1,$allmenu=0) {
